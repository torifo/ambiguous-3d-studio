 ...
 * 生成の逆順に破棄する。リーク判定は生存オブジェクト数
 * （`getLiveWasmObjectCount`）で行う。Emscripten のヒープは `delete()` しても
 * 縮まないため、ヒープ容量を見る判定は必ず偽陽性になる。
 */
 ...
 * 2. `.extrude(depth, 0, 0, [1, 1], true)` — **center = true 必須**。
 *    忘れると片方が Z=0 から始まり、交差が非対称に切り落とされる
...
export function performCsg(wasm: ManifoldToplevel, request: CsgRequest): CsgResponse {
  ...
  try {
    ...
    rawA = track(sectionA.extrude(request.a.depth, 0, 0, [1, 1], true))
    rawB = track(sectionB.extrude(request.b.depth, 0, 0, [1, 1], true))

    // A は回転しない（XY 断面のまま +Z 押し出し）。B のみ +90° で +X へ
    const prismA = rawA
    prismB = track(rawB.rotate([0, 90, 0]))
    ...
    // 生成の逆順に破棄：parts[]（逆順）→ solid → prismB → rawB → rawA
    // → sectionB → sectionA。decompose() の戻り配列が最も漏れやすい
    if (parts !== null) {
...
interface CsgWorkerScope {
  ...
  postMessage(message: CsgResponse, transfer?: Transferable[]): void
}
...
if (isWorkerScope()) {
  ...
  // 起動と同時に Wasm を先読みする（NFR-003）。初期化失敗は最初のリクエスト
  // 処理時に WASM_INIT_FAILED として報告されるため、ここでは握りつぶす
  ensureManifold().catch(() => undefined)
