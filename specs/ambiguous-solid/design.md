...
| プリフライト（256 走査線） | 10ms |
| postMessage 往復 | 5ms |
| CrossSection 構築 ×2 | 20ms |
| extrude ×2 | 30ms |
| intersect | 150ms |
| MeshGL コピー + 転送 | 15ms |
| BufferGeometry 構築 + 法線 | 20ms |
| **合計** | **257ms** |

残余 43ms がバッファ。SVG 由来の高頂点数入力ではこの予算を超えうるため、頂点数上限（10,000）と曲線フラット化の許容誤差で入力側を抑える。Wasm 初期化（実測 100〜300ms）は起動時に先読みし予算外（NFR-003）。
